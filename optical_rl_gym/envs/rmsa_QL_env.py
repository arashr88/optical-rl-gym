...
class RMSAQLEnv(OpticalNetworkEnv):
    ...
    def __init__(
        ...
        num_actions_k_paths = self.k_paths + self.reject_action
        num_actions_spectrum_resources = self.num_spectrum_resources + self.reject_action

        total_num_actions = num_actions_k_paths * num_actions_spectrum_resources

        self.action_space = gym.spaces.Discrete(num_actions_k_paths)
        ...
        #     }
        # )
        # self.observation_space = gym.spaces.MultiDiscrete([num_actions_spectrum_resources] * num_actions_k_paths)
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(num_actions_k_paths, num_actions_spectrum_resources), dtype=np.float32)
        self.action_space.seed(self.rand_seed)
        self.observation_space.seed(self.rand_seed)

        self.logger = logging.getLogger("rmsaqlenv")
    ...
    def step(self, action):
        path, initial_slot = action, first_fit(self, action)
        self._release()
    ...
    def observation(self):
        new_obs = np.ones(
            (self.k_paths , self.num_spectrum_resources ), dtype=int
        )
        for idp, path in enumerate(
            ...
            for nodei in range(path.hops):
                new_obs[idp] =  [a & b for a, b in zip(new_obs[idp], self.topology.graph["available_slots"][self.topology[path.node_list[nodei]][path.node_list[nodei + 1]]["index"]] )]
    ...
    def reward(self, idp):
        ...
        for nodei in range(path.hops):
            new_obs =  [a & b for a, b in zip(new_obs, self.topology.graph["available_slots"][self.topology[path.node_list[nodei]][path.node_list[nodei + 1]]["index"]] )]
        num_slots = np.sum(new_obs) + self.get_number_slots(  path  )
    ...
    def _update_link_stats(self, node1: str, node2: str):
        ...
        if self.current_time > 0:
            ...
            if np.sum(slot_allocation) > 0:
                initial_indices, values, lengths = RMSAQLEnv.rle(slot_allocation)
                ...
                if len(used_blocks) > 1:
                    ...
                    internal_idx, internal_values, internal_lengths = RMSAQLEnv.rle(
    ...
    def get_available_blocks(self, path):
        ...
        # getting the blocks
        initial_indices, values, lengths = RMSAQLEnv.rle(available_slots)
    ...
    def _get_network_compactness(self):
        ...
        for n1, n2 in self.topology.edges():
            # getting the blocks
            initial_indices, values, lengths = RMSAQLEnv.rle(
                self.topology.graph["available_slots"][
...
def shortest_path_first_fit(env: RMSAQLEnv) -> Tuple[int, int]:
...
def first_fit(env: RMSAQLEnv, idp) -> Tuple[int, int]:
    num_slots = env.get_number_slots(
        env.k_shortest_paths[
            env.current_service.source, env.current_service.destination
        ][idp]
    ...
    for initial_slot in range(
        ...
        if env.is_path_free(
            ...
            return initial_slot
    return env.topology.graph["num_spectrum_resources"]

def shortest_available_path_first_fit(env: RMSAQLEnv) -> Tuple[int, int]:
    for idp, path in enumerate(
        env.k_shortest_paths[
            env.current_service.source, env.current_service.destination
        ]
...
def least_loaded_path_first_fit(env: RMSAQLEnv) -> Tuple[int, int]:
    max_free_slots = 0
    action = (
...
def least_congested_path_first_fit(env: RMSAQLEnv) -> Tuple[int, int]:
    max_free_slots = 0
    action = (
        env.topology.graph["k_paths"],
        env.topology.graph["num_spectrum_resources"],
...
def dqn_routing(self):
    return

def least_congested_path_KSP_first_fit(env: RMSAQLEnv) -> Tuple[int, int]:
    max_free_slots = 0
    action = (
        env.topology.graph["k_paths"],
        env.topology.graph["num_spectrum_resources"],
...
class SimpleMatrixObservation(gym.ObservationWrapper):
    def __init__(self, env: RMSAQLEnv):
        super().__init__(env)
        shape = (
            self.env.k_paths
