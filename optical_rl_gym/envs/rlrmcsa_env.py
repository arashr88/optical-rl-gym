...
class RLRMCSAEnv(OpticalNetworkEnv):
    ...
    def __init__(
        ...
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(self.num_spatial_resources , self.topology.number_of_edges(), self.num_spectrum_resources  ), dtype=np.uint8)
        """
        self.action_space = gym.spaces.Discrete(
        ...
        self.logger = logging.getLogger("rlrmcsaenv")
    ...
    def _update_link_stats(self, core: int, node1: str, node2: str):
        ...
        if self.current_time > 0:
            ...
            if np.sum(slot_allocation) > 0:
                initial_indices, values, lengths = RLRMCSAEnv.rle(slot_allocation)
                ...
                if len(used_blocks) > 1:
                    ...
                    internal_idx, internal_values, internal_lengths = RLRMCSAEnv.rle(
    ...
    def _get_network_compactness(self, core):
        ...
        for n1, n2 in self.topology.edges():
            # getting the blocks
            initial_indices, values, lengths = RLRMCSAEnv.rle(
                self.topology.graph["available_slots"][
...
def shortest_path_best_modulation_first_core_first_fit(env: RLRMCSAEnv) -> int:
    """
    Algorithm for determining the shortest available first core first fit path
...
class SimpleMatrixObservation(gym.ObservationWrapper):
    def __init__(self, env: RLRMCSAEnv):
        super().__init__(env)
        shape = (
